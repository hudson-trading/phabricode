...
export class PhabNodeProvider implements vscode.TreeDataProvider<ReviewDependency|TopLevelDependency|CommentDependency> {
	...
	constructor(commentController: vscode.CommentController) {
		...
		if (config.arcrc.host !== null && config.arcrc.token !== null) {
			this.phabUrl = config.arcrc.host;
			this.phabToken = config.arcrc.token;
		} else {
		...
		*/
		...
		this.phabApi = axios.create({
			baseURL: this.phabUrl,
			timeout: 5000,
			headers: {
				'Content-Type': 'application/xjwww-form-urlencoded', 'Accept': 'application-type/json'}
			});
	}

	async post (api: string, params: object) {
		...
		let data = `api.token=${this.phabToken}`;
		for (const paramKey in params) {
			data += `&${paramKey}=${params[paramKey]}`
		}
		return this.phabApi.post(api, data)};
	...
	setup(): void {
		...
		if (Object.keys(config.workspace.repoCallSignToWorkspaceFolder).length > 0) {
			...
			this.post('diffusion.repository.search', constraints).then(repoQuery => {
				repoQuery.data.result.data.forEach(entry => {
	...
	private async getUsernameFromAuthorPhid(phid: string): Promise<string> {
		if (!this.authorPhidToUsername.has(phid)) {
			...
			const user = await this.post('user.query',  filter);
			this.authorPhidToUsername.set(phid, user.data.result[0].userName);
		}
	...
	public async loadPhabFromUserInput() {
		...
		if (input === undefined) {
			vscode.window.showErrorMessage("Phabricode: please provide a value");
			return
		}
		...
		if (match !== null && match.length > 0) {
			...
			const revisions = await this.post('differential.revision.search', filter);
			if (revisions.data.result.data.length > 0) {
				const phid = revisions.data.result.data[0].phid;
	...
	public async gitCheckout(revdep: ReviewDependency) {
		const filterA = {'constraints[phids][0]': revdep.phid};
		const rev = await this.post( 'differential.revision.search', filterA);
		const diffPhid = rev.data.result.data[0].fields.diffPHID;
		const filterB = {'constraints[phids][0]': diffPhid};
		const diff = await this.post( 'differential.diff.search', filterB);
		const branch = diff.data.result.data[0.].fields.refs.find(item => item.type === "branch").name
		console.log(`${revdep.phid} maps to branch ${branch}`)
		await vscode.commands.executeCommand("git.checkout", "repo", branch);
		// let termName = "git checkout";
		...
		// })
		// term.sendText(`git checkout ${branch}`);
		// term.sendText("exit");
	}
	...
	public async loadPhab(phid: string) {
		...
		*/
		...
		const rev = await this.post( 'differential.revision.search', filterA);
		const objKey = `D${rev.data.result.data[0].id}.md`;
		const uri = vscode.Uri.parse(`phab:${objKey}`);
		const filterB = {'objectIdentifier': phid};
		const phab = await this.post( 'transaction.search', filterB);
		...
		*/
		...
		const itemPromises = phab.data.result.data.filter(item => item.comments.length >0).map(item => this.processItem(item));
		...
		if (vscode.workspace.workspaceFolders !== undefined) {
			uriRoot = vscode.workspace.workspaceFolders[0].uri.fsPath;
		...
		*/
		...
		this.phabsToContent[objKey] = this.formatPhabContent(rev.data.result.data[0]);
...
`;
	...
	private async getReviews(depType: DepType): Promise<ReviewDependency[]> {
		if (!this.userPhib) {
			const userData = await this.post('user.whoami', {});
			if (userData === undefined) {
				vscode.window.showErrorMessage(`Unable to fetch user data when invoking user.whoami on ${this.phabUrl} with token ${this.phabToken}`);
			}
			this.userPhib = userData.data.result.phid;
		}
		let filters = {};
		switch (depType) {
		...
		const phabs = await this.post('differential.revision.search', filters);
		const phabPromises = phabs.data.result.data.map(phab => this.phabToReviewDependency(phab));
